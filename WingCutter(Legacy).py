from numpy import array, sin, cos, tan, arctan, hypot, sign, append, max, arctan2, sqrt, radians, around, zeros, unique, matmul, uint8, argmax, argmin, delete, dot, mean, arctan2, argmax, roll, absolute, pi, equal, empty, arange, isnan, NINF, Inf, r_, linspace, stack
...
class CutterTools():
    ...
    def slice(self,p1,p2,wing_span,x_offset,cutter_width=890):
        ...
        for self.index in range(self.plane1.shape[0]): #uses a paramerterisation to compute the coordinates
            ...
            self.der = (self.pair[1]-self.pair[0])/wing_span
            self.left_tower_param = (0-x_offset)/self.der[2]
            self.right_tower_param = (cutter_width-x_offset)/self.der[2]
...
class GUI():
    def __init__(self):
        ...
        self.sliceandsave_button = Button(self.window,text="Add to cutting plan",command = self.sliceandsave)
        self.sliceandsave_button.place(x=420,y=320)
    ...
    def gen_naca(self, foil_num):#Bug some where here
        ...
        for x in range(0,100,1):
            ...
            self.y_a = self.camber_offset-self.thickness*cos(self.offset_theta)
            self.foil_surface.append([self.x_a,self.y_a])
        self.foil_surface = array([[self.foil_num],self.foil_surface])
    ...
    def generate_root_foil(self):
        if self.root_foil_num.get() != "":
            self.root_info, self.root_data = self.gen_naca(self.root_foil_num.get())
        ...
        self.root_info = r_[self.root_info,[int(self.root_alpha_input.get()),float(self.root_twist_percentage_input.get()),float(self.Root_chord_input.get())]]
    ...
    def generate_tip_foil(self):
        if self.tip_foil_num.get() !="":
            self.tip_info, self.tip_data = self.gen_naca(self.tip_foil_num.get())
        ...
        self.tip_info = r_[self.tip_info,[float(self.tip_alpha_input.get()),float(self.tip_twist_percentage_input.get()),float(self.tip_chord_input.get())]]
